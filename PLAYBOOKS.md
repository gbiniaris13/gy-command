...
### Instagram — yacht-first feed (Boss directive 2026-05-07)

The IG pipeline is intentionally serial so Mon-Thu are reserved for
yacht posts (instagram-fleet-post, rotation through 66 yachts in
Sanity, see [`src/lib/fleet-rotation.ts`](src/lib/fleet-rotation.ts)).
Fri-Sun fall through to instagram-publish, which drains the queue
generated by instagram-generate-weekly. Net cadence: **4 yacht +
3 scenery per week**.

| Time (UTC) | Endpoint | Purpose |
|---|---|---|
| `0 15 * * 1,2,3,4` | **instagram-fleet-post** | **Mon-Thu yacht post (claims daily slot first)** |
| `30 15 * * *` | instagram-publish | Daily — picks oldest queued (`ascending: true`) for Fri/Sat/Sun |
| `5 15 * * 1,4` | instagram-carousel | Mon/Thu carousel (flag-gated, default off) |
| `15 15 * * 3,5` | instagram-publish-reel | Wed/Fri reel publish |
| `0 7 * * 0` | instagram-generate-weekly | Sunday — generates 3 captions for Fri/Sat/Sun only (Mon-Thu skipped via FLEET_DAY_INDICES) |
| `0 5,10,17 * * *` | instagram-stories | 3×/day stories |
| `0 */3 * * *` | instagram-fleet-story-followup | Story queue drainer |
| `17 */6 * * *` | instagram-analytics | 4×/day analytics pull |
| `30 6 * * *` | instagram-watchdog | Stuck-post detection |
| `0 7 * * 1` | instagram-health-check | Monday health check |
| `0 7 * * 4` | instagram-weekly-ops-report | Thursday Telegram summary |
| `0 7 1 * *` | instagram-evergreen | Monthly evergreen |
| `0 8 1 * *` | instagram-monthly-report | Monthly report |
| `11 3 * * *` | instagram-followers | Daily follower delta |

**Critical ordering invariant.** instagram-fleet-post runs at **15:00
UTC** (18:00 Athens), instagram-publish at **15:30 UTC**. fleet-post
must claim the daily slot first or the rate-limit guard
(`assertPublishAllowed`) blocks it on Mon-Thu and the feed reverts
to all-scenery. Don't reorder these timings without re-reading the
2026-05-07 incident commit (gy-command `ad2f0b9`).

**Rotation flags (settings table — flip via `/api/admin/setting`).**

| Key | Value | What it controls |
|---|---|---|
| `fleet_posts_enabled` | `"true"` | Master switch for the yacht rotation feed. Default off. |
| `fleet_posts_per_week` | `"4"` | Per-week cap inside fleet-post. 4 matches the 4-day Mon-Thu schedule. |
| `carousel_auto_publish_enabled` | `"false"` | Carousel auto-publish (default off — flag-gated since Apr 2026 incident with template carousel). |

**Cycle-through-66 behaviour.** The rotation state lives at
`settings.fleet_rotation_v1` (JSON: `byYacht[id].lastPostedAt +
angleHistory`, plus `lastYachtId`). 14-day cooldown per yacht; if
all 66 are on cooldown the least-recently-posted wins (forces
rotation forward). 6 angle types per yacht (inside_info,
ideal_guest, toys_tour, builder_heritage, cruising_canvas,
crew_spotlight) so even repeat yachts get fresh framing. New
yachts added in Sanity automatically join the pool with no
`lastPostedAt`, jumping straight to priority on the next tick.
