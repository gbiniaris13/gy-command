...
// v3 Pillar 7 — surfaces today's + this-week's charter lifecycle
// milestones in the cockpit. Each row tells George which deal needs
// what touchpoint and whether a Gmail draft was already generated by
// the daily firing cron.
export interface CharterMilestoneRow {
  id: string;
  deal_id: string;
...
export interface ChartersReady {
  due_today_count: number;
...
export interface CockpitBriefing {
  ...
  /** v3 Pillar 7 — today's + this-week's charter lifecycle touchpoints.
   *  Drafted automatically by the daily firing cron; surfaced here so
   *  George can review-and-send first thing in the morning. */
  charters_ready: ChartersReady;
...
Generate ONE devil's-advocate provocation for George. NOT a coaching prompt — a challenge.`;
  ...
  // Wrapped in try/catch so the cockpit gracefully no-ops if the v3
  // schema migration hasn't been applied yet.
  let chartersReady: ChartersReady = {
    due_today_count: 0,
    due_this_week_count: 0,
    drafted_count: 0,
    rows: [],
    gmail_label_url: "https://mail.google.com/mail/u/0/#label/gy-charter-lifecycle",
  };
  try {
    ...
    const { data: rawMs } = await sb
      .from("charter_lifecycle_milestones")
      .select(
        "id, deal_id, milestone_type, due_date, status, auto_action, gmail_draft_id",
      )
    ...
    const msList = (rawMs ?? []) as Ms[];

    if (msList.length > 0) {
      const dealIds = Array.from(
        new Set(msList.map((m) => m.deal_id).filter(Boolean)),
      ) as string[];
      const { data: deals } = await sb
        .from("deals")
        .select(
          "id, primary_contact_id, vessel_name, charter_start_date, charter_end_date",
        )
        .in("id", dealIds);
      type D = {
        id: string;
      ...
      const dealById = new Map<string, D>(
        ((deals ?? []) as D[]).map((d) => [d.id, d]),
      );
      ...
      let contactById = new Map<string, C>();
      if (contactIds.length) {
        const { data: contacts } = await sb
          ...
          .in("id", contactIds);
        contactById = new Map(
          ((contacts ?? []) as C[]).map((c) => [c.id, c]),
        );
      }

      const rows: CharterMilestoneRow[] = msList
        .map((m) => {
          const d = m.deal_id ? dealById.get(m.deal_id) : null;
          const c = d?.primary_contact_id
            ? contactById.get(d.primary_contact_id)
            : null;
          const name = c
          ...
          return {
            ...
            gmail_draft_id: m.gmail_draft_id,
            charter_start_date: d?.charter_start_date ?? null,
            charter_end_date: d?.charter_end_date ?? null,
          };
        })
        .filter((r) => r.deal_id);

      chartersReady = {
        due_today_count: rows.filter((r) => r.due_date <= today).length,
        due_this_week_count: rows.length,
        drafted_count: rows.filter((r) => r.gmail_draft_id).length,
        rows,
        gmail_label_url:
          "https://mail.google.com/mail/u/0/#label/gy-charter-lifecycle",
      };
    }
  } catch (e) {
    void e; // v3 migration not applied yet
  }
